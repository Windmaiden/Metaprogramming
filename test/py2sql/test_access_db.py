...
class TestGeneralAccess(unittest.TestCase):
    db_config = py2sql.DBConnectionInfo("test", "localhost", "postgres", "postgres")
    ...
    def test_run_all_readonly_functions(self):
        py2sql.Py2SQL.db_connect(self.db_config)
        self.assertEqual(py2sql.Py2SQL.db_engine(),['PostgreSQL', '13.1,'])
        self.assertEqual (py2sql.Py2SQL.db_name(), "test")
        self.assertGreater(py2sql.Py2SQL.db_size(),0)
